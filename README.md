...
This tool supports OpenTelemetry (OTel) for tracing agent reasoning, tool execution, and security workflows.

### Local Visualization with Jaeger

You can visualize the entire thinking process of the AI agent using Jaeger:
...
2. **Run with OTel Endpoint**:
   Set the `OTEL_EXPORTER_OTLP_ENDPOINT` environment variable to point to the Jaeger collector (HTTP/JSON):
   ```bash
   export OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4318
   llsc "Your prompt here"
   ```

3. **View Traces**:
   Open http://localhost:16686 in your browser. You can see detailed spans for:
   - LLM "Thinking" durations
   - Tool calling arguments and results
   - Security verification phases (CASS, Dual LLM)

If the environment variable is not set, the tool operates with standard console logging only.
...
### OpenTelemetryによる可視化（トレーシング）

本ツールは OpenTelemetry (OTel) をサポートしており、エージェントの思考プロセス、ツール実行、セキュリティ検証の各ステップを詳細に可視化できます。

1. **Jaegerの起動** (Dockerを使用):
   ```bash
   docker run -d --name jaeger \
     -p 4318:4318 \
     -p 16686:16686 \
...
2. **OTelエンドポイントを指定して実行**:
   環境変数 `OTEL_EXPORTER_OTLP_ENDPOINT` に Jaeger のコレクター（HTTP/JSON）を指定します。
   ```bash
   export OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4318
   llsc "プロンプトを入力"
   ```
...
3. **トレースの確認**:
   ブラウザで http://localhost:16686 を開くと、以下のステップをガントチャート形式で確認できます：
   - LLMの思考時間
   - ツール呼び出しの引数と実行結果
   - セキュリティ検証（CASS、Dual LLM）の各フェーズ

環境変数が設定されていない場合、通常のコンソールログ出力のみで動作します。
