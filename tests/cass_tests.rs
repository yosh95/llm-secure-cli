...
fn test_cass_risk_evaluation() {
    ...
    let risk = CASS_ORCHESTRATOR.evaluate_risk("execute_command", &config);
    ...
    let risk = CASS_ORCHESTRATOR.evaluate_risk("create_or_overwrite_file", &config);
    assert!(matches!(risk, RiskLevel::High));
}
...
fn test_cass_security_posture_requirements() {
    ...
    let posture = CASS_ORCHESTRATOR.get_security_requirements("execute_command", &config);
    ...
    let posture = CASS_ORCHESTRATOR.get_security_requirements("list_files", &config);
    assert!(posture.require_pqc_signature);
