...
fn test_audit_hash_chaining() {
    ...
    log_audit("test_tool", serde_json::json!({}), None, Some(0), None, None);
    log_audit("test_tool", serde_json::json!({}), None, Some(0), None, None);
    ...
    if lines.len() >= 2 {
        ...
        assert_eq!(hash1, prev_hash2, "Hash chain broken! Prev hash of second entry should match hash of first entry.");
        assert_ne!(prev_hash2, "0".repeat(64), "Prev hash should not be all 0s for second entry");
    } else {
        panic!("Should have at least 2 audit entries, found {}", lines.len());
    }
