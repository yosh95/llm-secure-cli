...
use crate::modules::models::{ClientState, DataSource, Message};
...
pub trait LlmClient: Send + Sync {
    ...
    fn load_session(&mut self, path: &str) -> anyhow::Result<()> {
        ...
        let conversation: Vec<crate::modules::models::Message> = serde_json::from_reader(file)?;
    ...
    fn update_history(&mut self, data: &[DataSource], model_msg: Message) {
        ...
        for d in data {
            if d.content_type == "text/plain" {
                if let Some(text) = d.content.as_str() {
                    user_parts.push(crate::modules::models::MessagePart::Text(text.to_string()));
                }
            } else {
                ...
                user_parts.push(crate::modules::models::MessagePart::Part(
                    crate::modules::models::ContentPart {
                        text: None,
        ...
        if !user_parts.is_empty() {
            self.get_state_mut()
                .conversation
                .push(crate::modules::models::Message {
                    role: crate::modules::models::Role::User,
                    parts: user_parts,
                });
        }
