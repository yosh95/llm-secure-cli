use crate::clients::session::ChatSession;
use crate::modules::models::Role;
use crate::ui;
...
pub async fn handle_command(session: &mut ChatSession, input: &str) -> CommandResult {
    ...
    match cmd.as_str() {
        ...
        "edit" | "e" => {
            match ui::open_external_editor(args) {
                Ok(content) => {
        ...
        "reload" => {
            crate::clients::config::CONFIG_MANAGER.reload();
...
pub async fn handle_attach(session: &mut ChatSession, source: &str) {
    ...
    let data = crate::modules::media_utils::process_single_source(source, true).await;
    if let Some(d) = data {
...
pub fn handle_provider_cmd(session: &mut ChatSession, args: &str) {
    if args.is_empty() {
        let active_providers = crate::clients::config::CONFIG_MANAGER.get_active_providers();
