...
pub async fn list_models(
    ...
    let (url, headers) = match provider.as_str() {
        ...
        _ => {
            ui::report_error(&format!(
                "Unknown or unsupported provider for listing: {}",
                provider
            ));
            return;
        }
    ...
    let result: Value = match res.json().await {
        Ok(j) => j,
        Err(e) => {
            ui::report_error(&format!("Error parsing models JSON: {}", e));
            return;
        }
    };
    ...
    if let Some(models_list) = models_data.and_then(|m| m.as_array()) {
        ...
        if !models.is_empty() {
            ...
            if provider == "openrouter" {
                println!("{:<50} {:<30}", "Model ID", "Details");
                println!("{:-<80}", "");
            } else {
