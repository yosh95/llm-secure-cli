...
pub fn ask_approval(command: &str, explanation: &str) -> bool {
    println!();
    let color = "yellow";
    println!("{}", "╭── TOOL EXECUTION REQUEST".color(color).bold());
    println!(
        "{} {}: {}",
        "│".color(color).bold(),
        "Explanation".bold().cyan(),
        explanation
    );
    println!(
        "{} {}: {}",
    ...
    println!("{}", "╰──────────────────────────".color(color).bold());

    Confirm::new()
        .with_prompt("Approve execution?")
        .default(false)
        .interact()
        .unwrap_or(false)
}
