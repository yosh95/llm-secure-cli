...
pub fn register_builtin_tools(r: &mut ToolRegistry, config_manager: &crate::config::ConfigManager) {
    ...
    if crate::tools::builtin::python::is_python_available() {
        maybe_register(
            r,
            "execute_python",
            "Execute arbitrary Python code. Multi-purpose tool for: file ops (read/write/edit/list), grep/search (re, fnmatch), web fetching (requests/urllib), data processing, AND arbitrary shell commands (via subprocess). NOT for web research—use brave_search.",
            json!({
                "type": "object",
                "properties": {
                    "code": {
