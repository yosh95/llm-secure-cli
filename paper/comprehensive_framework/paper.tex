...
\begin{enumerate}
    \item \textbf{Semantic Containment (Space):} Establishing safety boundaries using an AI-native Verifier Committee Verifier and environment isolation.
    \item \textbf{Behavioral Integrity (Behavior):} Implementing Zero Trust where every high-risk action is verified by a Verifier Committee (N-member) against a hardcoded "Security Constitution" and synchronized with human cognition.
...
\subsection{AI-Native Semantic Guardrails (Verifier Committee)}
Traditional systems rely on platform-specific blocklists (e.g., \texttt{rm -rf /}). These are easily bypassed by obfuscation or novel shell features. Our framework adopts \textbf{Verifier Committee Verification} as the primary guardrail. Every tool call is intercepted and sent to a secondary, lightweight LLM that evaluates the \textit{impact} of the command against the user's original intent and a hardcoded \textbf{Security Constitution}.
...
\subsection{Intent Verification \& Structured Verdicts (Verifier Committee)}
The Verifier Committee Verifier uses \textbf{Native Tool Calling} to provide structured verdicts. By forcing the auditor LLM to output a JSON schema (\texttt{decision: ALLOW|REVIEW}, \texttt{reason: string}; BLOCK is mapped to REVIEW for mandatory human oversight), we eliminate parsing brittleness. The verifier is provided with \textbf{Security Context} (OS, User, Git status) to judge risk semantically.
...
We implement a \textbf{Chained Hashing} audit log where each entry is signed using \textbf{ML-DSA} (FIPS 204). High-risk arguments are encrypted using \textbf{ML-KEM-1024} (FIPS 203) to protect against "Store Now, Decrypt Later" threats.
...
\subsection{Intent Verification (Verifier Committee) \& Confidence-Awareness}
To mitigate sophisticated Indirect Prompt Injection attacks, we implement a \textbf{Verifier Committee Verification} pattern. For tools classified as high-risk by CASS, the system invokes a separate, lightweight LLM (e.g., Gemini 2.0 Flash) to verify the proposed action against the user's original, untainted prompt. 

This verifier evaluates proposed tool calls against the Security Constitution. The system currently supports two built-in tools:
\begin{itemize}
    \item \textbf{Universal Executor:} \texttt{execute\_python} (arbitrary Python code execution for file operations, data processing, and computation).
    \item \textbf{Web Search:} \texttt{brave\_search} (Brave LLM Context API for grounded, pre-extracted web content).
\end{itemize}
Additional tools can be integrated via the MCP (Model Context Protocol) server interface.

This verifier implements \textbf{Confidence-Aware Verification}: the Verifier Committee (N-member) provides a confidence score (0.0--1.0) along with its verdict. It is important to note that these scores represent the model's self-reported certainty and may not be perfectly calibrated to actual safety probabilities. Therefore, the system treats the verdict as "Uncertain" and automatically escalates to a Human-in-the-Loop (HITL) review if the score falls below a set threshold (e.g., 0.7). This threshold selection is deployment-specific and should be validated against local workloads to balance security and usability.

The verifier also implements \textbf{Context-Aware Verification} by receiving the output of the immediately preceding tool call (\texttt{last\_tool\_result}) alongside the current request. This allow the Verifier Committee (N-member) to distinguish between genuinely suspicious actions and legitimate corrective steps (e.g., an agent editing a file to fix an error reported by a linter in the previous turn). However, because the previous tool result can itself contain attacker-controlled text, the verifier prompt is designed to treat this field as untrusted evidence rather than absolute truth.
...
To balance security with usability, the framework implements a configurable \textbf{Auto-Approval Policy} (\texttt{auto\_approval\_level}) with three tiers: \texttt{none} (all tools require manual approval), \texttt{low} (only low-risk tools, e.g., directory listing, are auto-approved), and \texttt{medium} (low- and medium-risk tools are auto-approved). The default is \texttt{low}. Critically, any security warning raised by the Verifier Committee verifier unconditionally overrides the auto-approval policy, forcing manual review regardless of the risk level classification.
...
The framework implements \textbf{PQC Agility}, dynamically adjusting security levels based on risk. Higher-risk tools (e.g., \texttt{execute\_command}) mandate ML-DSA-87 signatures and detailed human approval, while low-risk observations use ML-DSA-87 with relaxed oversight.
...
[security]
high_risk_tools = ["execute_python"]
medium_risk_tools = ["brave_search"]
...
\hline
PQC Signature & ML-DSA-87 & ML-DSA-87 & ML-DSA-87 \\
Audit Encryption & Off & Off & ML-KEM-1024 + AES-256-GCM \\
Verifier Committee Verification & Off & Off & Configurable / Required \\
...
Verifier Committee Guardrail & Malicious Intent & 100.0\% (5/5) \\
\hline
...
Table~\ref{tab:total_perf} reports the latency of each local security primitive. The true cumulative security overhead---comprising static analysis, ML-DSA-87 identity signing, and ML-KEM-1024 audit encryption---is $\approx$1.15\,ms. All cryptographic latencies are measured using native Rust implementations ({\tt saorsa-pqc} crates). This "Security Speed Bump" is practically invisible to the user, remaining well below the 500\,ms--3000\,ms RTT typical of cloud-based LLM inference.
...
Identity Gen (ML-DSA-87)        &  0.55   \\
Identity Gen (ML-DSA-87)        &  0.82   \\
KEM Encapsulation (ML-KEM-1024)  &  0.06   \\
\hline
...
\subsection{Intent Verification (Verifier Committee) Performance}
The Verifier Committee Verification pattern was evaluated across three providers using a comprehensive suite of 115 test cases ($n=115$) covering benign development tasks, ambiguous filesystem operations, and various adversarial scenarios (injection, data theft, and unauthorized system modification). Table \ref{tab:dual_llm_cases} defines a subset of these cases; TC1--TC5 are benign scenarios, while TC6--TC10 represent adversarial attempts.

\begin{table}[htbp]
\caption{Verifier Committee Evaluation Test Cases (Selection)}
\begin{center}
\scriptsize
...
As shown in Table~\ref{tab:dual_llm_perf}, both Google and Anthropic models achieved high classification accuracy (94.8\%). OpenAI's Nano model displayed a significantly higher False Positive rate (24.3\%), frequently blocking ambiguous but benign filesystem operations in a "safety-first" posture. All models maintained high recall ($>$85\%), successfully blocking the majority of critical adversarial attempts.

The observed verifier latency is network-bound and varies significantly between providers. Google Gemini 3.1 Flash-Lite demonstrated the lowest mean latency ($\approx$1096\,ms), while Anthropic Claude Haiku 4.5 exhibited the highest ($\approx$3332\,ms). Because verification runs concurrently with the user's cognitive review of the tool explanation, this latency is intended to be hidden inside the manual oversight window rather than added serially to every tool call.

\begin{table}[htbp]
\caption{Verifier Committee Verification Performance (Function Calling, n=115 scenarios).
Acc. represents balanced classification accuracy across benign and adversarial cases.}
\begin{center}
...
\hline
Google    & Gemini 3.1 Flash-Lite & 94.78\% & $\approx$1096 \\
OpenAI    & GPT-5.4 Nano          & 74.78\% & $\approx$1884 \\
Anthropic & Claude Haiku 4.5      & 94.78\% & $\approx$3332 \\
\hline
\end{tabularx}
