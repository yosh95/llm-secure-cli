...
To mitigate sophisticated Indirect Prompt Injection attacks, we implement a \textbf{Dual LLM Verification} pattern. For tools classified as high-risk by CASS, the system invokes a separate, lightweight LLM (e.g., Gemini 2.0 Flash) to verify the proposed action against the user's original, untainted prompt. 

This verifier facilitates autonomous navigation through a comprehensive toolset including:
\begin{itemize}
    \item \textbf{Search \& Discovery:} \texttt{grep\_files} (regex-based content search) and \texttt{search\_files} (filename pattern discovery).
    \item \textbf{Precision Modification:} \texttt{edit\_file} (indentation-aware block replacement) and \texttt{create\_or\_overwrite\_file}.
    \item \textbf{Observation \& Context:} \texttt{read\_url\_content} (SSRF-protected web crawling) and \texttt{read\_many\_files} (batch context loading).
    \item \textbf{System Agency:} \texttt{execute\_command} (direct binary execution).
\end{itemize}
...
The verifier also implements \textbf{Context-Aware Verification} by receiving the output of the immediately preceding tool call (\texttt{last\_tool\_result}) alongside the current request. This allow the secondary LLM to distinguish between genuinely suspicious actions and legitimate corrective steps (e.g., an agent editing a file to fix an error reported by a linter in the previous turn). However, because the previous tool result can itself contain attacker-controlled text, the verifier prompt is designed to treat this field as untrusted evidence rather than absolute truth.
...
[security]
high_risk_tools = ["edit_file", "execute_command"]
medium_risk_tools = ["read_file_content", "grep_files", "read_url_content"]
...
\begin{table}[htbp]
\caption{Dual LLM Verification Performance (Function Calling, n=115 scenarios).
