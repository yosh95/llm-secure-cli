...
From an implementation perspective, this dual verification is performed concurrently with the user's review of the tool's rationale. By overlapping the verification latency with the human-in-the-loop stage, the framework effectively masks the computational overhead of the intent check. This approach ensures that the additional security layer does not introduce perceptible delays in the agent-user interaction loop.
...
\section{Evaluation}
We evaluated the framework on two reference platforms: (1) an AMD Ryzen 5 8540U (Zen 4, 16\,GB RAM) running Python 3.12 under WSL2 on Windows, and (2) an Intel Core i7-7700 (Kaby Lake, 16\,GB RAM) running Python 3.12 on bare-metal native Linux. In Table~\ref{tab:total_perf}, the column \textit{WSL2 (Ryzen 5 8540U)} reports measurements from the first environment, and \textit{Bare-metal (i7-7700)} reports measurements from the second.

A noteworthy observation is that the WSL2 environment recorded lower ML-DSA latencies ($\approx$684--706\,ms) than the bare-metal i7-7700 ($\approx$1234--1248\,ms). This counter-intuitive result is attributed to two platform-specific factors: (a) the Ryzen 5 8540U benefits from the newer Zen 4 microarchitecture, which offers substantially higher single-thread IPC for the integer-heavy lattice arithmetic in pure-Python ML-DSA; and (b) WSL2 on a laptop platform introduces variable thermal and scheduler behavior that, in this test configuration, did not penalize compute-bound workloads. The WSL2 figures should therefore be read as \emph{indicative} of Zen 4 performance characteristics rather than as a comparison of virtualisation overhead. Readers deploying on older hardware should refer to the bare-metal i7-7700 column as a conservative baseline.
...
Table \ref{tab:security_eval} summarizes the detection capabilities across 25 author-designed test vectors (including 4 distributed trust scenarios). The framework achieved 100\% mitigation across all adversarial categories \textit{within this controlled test suite}. The five benign pass-through cases in the AST tier and five additional benign cases in the Dual LLM tier yielded a combined false positive rate of 0\% (0/10), confirming that legitimate code is not incorrectly blocked. These results should be interpreted as a demonstration of architectural correctness on representative scenarios, not as a statistically exhaustive security guarantee. Notably, the White-box Agency layer enabled users to abort 100\% of identified reasoning loops (repetitive trial-and-error cycles) within the first three tool calls, demonstrating the practical value of cognitive synchronization.

\begin{table}[htbp]
\caption{Security Effectiveness (n=25 test cases)}
\begin{center}
...
Tier 1 (AST) & Benign Pass-through & 100.0\% (5/5) \\
...
Table~\ref{tab:total_perf} reports the latency of each local security primitive. The \textit{subprocess baseline} row is included for reference to characterise the execution environment, but is \emph{not} counted as security overhead, since process spawning occurs independently of the framework's security stack. The true cumulative security overhead---comprising AST analysis, ML-DSA-87 identity signing, and ML-KEM-768 audit encryption---is $\approx$710\,ms on WSL2 (Ryzen 5 8540U) and $\approx$1252\,ms on bare-metal (i7-7700). All cryptographic latencies are measured from pure-Python reference implementations ({\tt dilithium-py}, {\tt kyber-py}); native C/Rust bindings would yield substantially lower values. Even at the higher bare-metal figure, this "Security Speed Bump" is bounded and one-time-per-tool-call, remaining well within the 500\,ms--3000\,ms RTT typical of cloud-based LLM inference.

\textit{Measurement Note on ABAC Overhead:} In both environments, the marginal overhead of adding ABAC claims to the identity token was found to be negligible. The measured difference was $-18.54$\,ms (WSL2, $\sigma=50.60$\,ms) and $-15.54$\,ms (bare-metal, $\sigma=64.25$\,ms). In both cases, $|\Delta| < \sigma$, confirming that the observed negative delta is attributable to measurement jitter and Python's garbage-collection cycles rather than a genuine speedup. The computational cost of policy-based claim injection is functionally zero relative to the underlying cryptographic signing operation.

\begin{table}[htbp]
\caption{Local Security Primitive Latency (ms). \textit{Subprocess baseline} shown for reference only; excluded from security totals.}
\begin{center}
\begin{tabularx}{\columnwidth}{|l|X|X|}
\hline
\textbf{Component} &
  \textbf{\shortstack{WSL2\\(Ryzen 5\\8540U)}} &
  \textbf{\shortstack{Bare-metal\\(i7-7700)}} \\
\hline
AST + Static Analysis           &  0.04   &  0.03   \\
Identity Gen (ML-DSA-44)        & 684.48  & 1233.73 \\
Identity Gen (ML-DSA-87)        & 706.16  & 1248.00 \\
KEM Encapsulation (ML-KEM-768)  &   3.39  &    4.40 \\
\hline
\textbf{Security Total}         & \textbf{709.59} & \textbf{1252.43} \\
\hline
\textit{Subprocess (ref.)}      & \textit{70.78}  & \textit{24.02}  \\
\hline
\end{tabularx}
...
As shown in Table~\ref{tab:dual_llm_perf}, all three providers achieved 100\% classification accuracy across both test environments. TC1--TC5 (benign) were correctly \emph{allowed} (false positive rate = 0/5 = 0\%), while TC6--TC10 (adversarial) were correctly \emph{blocked} (detection rate = 5/5 = 100\%). A note on the raw benchmark output: the \texttt{PASS} label in the benchmark log denotes \emph{test-case outcome}---i.e., the verifier's decision matched the expected label---not that the tool call was allowed. Adversarial cases TC6--TC10 record \texttt{PASS} precisely because the verifier correctly returned \texttt{safe=false} and blocked execution. These results are based on author-designed test vectors and should be interpreted as a demonstration of correctness on representative cases, not as a statistically exhaustive empirical benchmark.

Mean latency was broadly consistent across platforms, as cloud API round-trip time dominates over local processing differences. On WSL2 (Ryzen 5 8540U), mean latencies were $\approx$1703\,ms (Google), $\approx$1608\,ms (OpenAI), and $\approx$1894\,ms (Anthropic). On bare-metal (i7-7700), the corresponding figures were $\approx$1871\,ms, $\approx$1851\,ms, and $\approx$1913\,ms respectively (Table~\ref{tab:dual_llm_perf}). As the verification process runs concurrently with the user's cognitive review of the tool's explanation, the measured latency is effectively masked within the manual oversight window. Consequently, the impact on the overall end-to-end response time is minimized, maintaining a responsive experience even when the most rigorous security settings are applied.

\begin{table}[htbp]
\caption{Dual LLM Verification Performance (n=10 per environment).
...
\textbf{Provider} & \textbf{Model} & \textbf{Acc.} &
  \textbf{\shortstack{WSL2\\(ms)}} &
  \textbf{\shortstack{Bare-metal\\(ms)}} \\
\hline
Google    & Gemini 3.1 Flash Lite & 100\% & $\approx$1703 & $\approx$1871 \\
OpenAI    & GPT-5.4 Nano          & 100\% & $\approx$1608 & $\approx$1851 \\
Anthropic & Claude Haiku 4.5      & 100\% & $\approx$1894 & $\approx$1913 \\
\hline
\end{tabularx}
...
{\small\textit{Reproducibility note:} Dual LLM latency figures represent the mean of 10 single-pass wall-clock measurements per provider per environment from a residential broadband connection, and are subject to network jitter and API-side queuing. Local PQC primitive latencies were obtained from the benchmark scripts included in the repository (\texttt{llm-secure-cli-benchmark-dual}) under the platform configurations described above. Model version identifiers correspond to the API snapshots available at the time of measurement; future model updates may affect both latency and detection behavior.}
