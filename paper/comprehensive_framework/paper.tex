...
Mean latency was broadly consistent with previous measurements, as cloud API round-trip time dominates over local processing differences. On the reference environment, mean latencies were $\approx$1297\,ms (Google), $\approx$1467\,ms (OpenAI), and $\approx$1295\,ms (Anthropic) (Table~\ref{tab:dual_llm_perf}). As the verification process runs concurrently with the user's cognitive review of the tool's explanation, the measured latency is effectively masked within the manual oversight window.

\begin{table}[htbp]
\caption{Dual LLM Verification Performance (n=3).
Acc.: 100\% (based on test suite). Latencies represent stable subsequent calls.}
\begin{center}
...
{\small\textit{Reproducibility note:} Dual LLM latency figures represent the mean of 10 single-pass wall-clock measurements per provider from a residential broadband connection. Local PQC primitive latencies were obtained from the Rust benchmark suite (\texttt{tests/benchmarks.rs}) under the ChromeOS Flex configuration described above. Model version identifiers correspond to the API snapshots available as of April 2026.}
