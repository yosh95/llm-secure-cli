...
Table~\ref{tab:total_perf} reports the latency of each local security primitive. The true cumulative security overhead---comprising static analysis, ML-DSA-87 identity signing, and ML-KEM-768 audit encryption---is $\approx$1.34\,ms. All cryptographic latencies are measured using native Rust implementations ({\tt pqcrypto} crates). This "Security Speed Bump" is practically invisible to the user, remaining well below the 500\,ms--3000\,ms RTT typical of cloud-based LLM inference.

\begin{table}[htbp]
\caption{Local Security Primitive Latency (ms). Measured on Intel(R) Core(TM) i5-7300U @ 2.60GHz, Debian 13 (Rust).}
\begin{center}
\begin{tabularx}{\columnwidth}{|l|C|}
\hline
\textbf{Component} & \textbf{Latency (ms)} \\
\hline
Static Analysis (Pattern-based) &  1.09   \\
Identity Gen (ML-DSA-44)        &  0.08   \\
Identity Gen (ML-DSA-87)        &  0.16   \\
KEM Encapsulation (ML-KEM-768)  &  0.02   \\
\hline
\textbf{Security Total}         & \textbf{1.35} \\
\hline
\textit{Subprocess (ref.)}      & \textit{32.50} \\
\hline
\end{tabularx}
...
Mean latency was broadly consistent with previous measurements, as cloud API round-trip time dominates over local processing differences. On the reference ChromeOS Flex environment, mean latencies were $\approx$1690\,ms (Google), $\approx$1540\,ms (OpenAI), and $\approx$2220\,ms (Anthropic) (Table~\ref{tab:dual_llm_perf}). As the verification process runs concurrently with the user's cognitive review of the tool's explanation, the measured latency is effectively masked within the manual oversight window.
...
\textbf{Provider} & \textbf{Model} & \textbf{Acc.} &
  \textbf{Latency (ms)} \\
\hline
Google    & Gemini 3.1 Flash-Lite & 100\% & $\approx$1690 \\
OpenAI    & GPT-5.4 Nano          & 100\% & $\approx$1540 \\
Anthropic & Claude Haiku 4.5      & 100\% & $\approx$2220 \\
\hline
\end{tabularx}
