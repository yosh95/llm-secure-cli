\begin{tikzpicture}[
    ...
    \node [below=0.1cm of cass, font=\tiny, text width=5em, text centered] {(Maximum Strength PQC)};
    \path [line] (request) -- (cass);

    % Tier 1: Space
    \node [component, fill=green!5, right=1.2cm of cass, yshift=3.2cm] (ast) {Static Analysis \\ \textit{(Control char / Null-byte)}};
    \node [component, fill=green!5, right=of ast] (path) {Path Guardrails \\ \textit{(Verifier Committee)}};
    ...
    % Tier 2: Behavior
    \node [component, fill=purple!5, right=1.2cm of cass, yshift=0.3cm] (identity) {Verifier Committee \\ \textit{(N-member, any-flag)}};
    \node [component, fill=purple!5, right=of identity] (workspace) {Workload Identity \\ \textit{(ABAC / COSE Token)}};

\node [tierbox, fit=(identity) (workspace), label={[title]above:Tier 2: Behavioral Zero-Trust (Behavior)}] (t2box) {};


    % Tier 3: Time
    \node [component, fill=red!5, right=1.2cm of cass, yshift=-2.8cm] (pqc_sig) {Hybrid PQC Sign \\ \textit{(ML-DSA-87)}};
    \node [component, fill=red!5, right=of pqc_sig] (pqc_kem) {Audit Encryption \\ \textit{(ML-KEM-1024)}};
