...
async fn main() -> anyhow::Result<()> {
    println!("{}", "=== Dual LLM Verification Benchmark ===".bold().cyan());
    ...
    let json_path = args.get(1).map(|s| s.as_str()).unwrap_or("benchmark/scenarios.json");
    ...
    for (p_alias, p_model) in providers {
        let has_key = llm_secure_cli::config::CONFIG_MANAGER.get_api_key(p_alias).is_some();
        if !has_key {
            println!("Skipping {} (No API Key found)", p_alias.yellow());
            continue;
        }

        println!("\n{}", format!("Testing Provider: {} ({})", p_alias, p_model).bold().green());
        println!("{:-<100}", "");
        ...
        for scenario in &scenarios {
            ...
            if is_correct {
                correct += 1;
                if safe { tn += 1; } else { tp += 1; }
            } else {
                if safe { fn_ += 1; } else { fp += 1; }
            }

            println!(
                "[{:<30}] Expected: {:<7} | Actual: {:<7} | Time: {:>4}ms | Result: {}",
                if scenario.label.len() > 30 { scenario.label[..27].to_string() + "..." } else { scenario.label.clone() },
                scenario.expected,
                if actual == "SAFE" { actual.blue() } else { actual.red() },
                elapsed,
                if is_correct { "PASS".green() } else { "FAIL".red() }
            );
        ...
        println!("  Accuracy : {:.2}% ({}/{})", (correct as f64 / total) * 100.0, correct, total);
        println!("  Avg Latency: {} ms", total_time / scenarios.len() as u128);
        println!("  Confusion Matrix: TP={}, TN={}, FP={}, FN={}", tp, tn, fp, fn_);
